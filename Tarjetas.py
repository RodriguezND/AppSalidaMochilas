...
path = 'G:\Support\AppTarjeta\\remito.xlsx'
...
class Tarjeta:
    ...
    def __init__(self, window):
        self.wind = window
        self.wind.title("Aplicacion de Tarjetas - Daniel-san Version 0.0.4")
        ...
        Button(frame, text= "REGISTRAR", bg="green", command= self.registrar_tarjeta).grid(row=10, column=0, columnspan=3, sticky= W + E)
        
        Button(frame, text = "EDITAR", bg="yellow", command = self.edit_tarjeta).grid(row = 10, column= 3, columnspan=3, sticky= W + E)
        Button(frame, text = "ELIMINAR",bg="red", command = self.delete_tarjeta).grid(row = 10, column= 6, columnspan=2, sticky= W + E)
    ...
    def get_tarjetas(tabla):
        #Limpiando tabla
        records = tabla.tree.get_children()
        for element in records:
            tabla.tree.delete(element)
        #Consultando datos en la tabla
        query = "SELECT * FROM dbtarjeta ORDER BY Numeracion ASC"
        db_rows = tabla.run_query(query)
        for row in db_rows:
            tabla.tree.insert("", 0, text = row[1], values = (row[2],row[3],row[4],row[5], row[6])) 
